...
4. 再パックされた新しいファイルを `encoded` フォルダからルートの `SaveData`（または `SaveData_S`）ディレクトリにコピーし、元のファイルを上書きします。
5. ゲームを起動します。

## 免責事項

**自己責任でご使用ください。** ゲームのセーブデータを変更すると、予期しないゲームの動作、クラッシュ、または不可逆的なセーブデータの破損を引き起こす可能性があります。
- 本ツールは自動バックアップを作成しようとしますが、使用前にセーブデータフォルダ全体を手動でバックアップすることを強くお勧めします。
